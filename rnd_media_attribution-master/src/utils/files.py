...
class CSVFile(BaseFile):
    ...
    def read(
        ...
        logger.debug(f"read data from {origin}")
        data = pd.read_csv(origin, chunksize=chunksize, dtype=dtypes)
        return data

    @staticmethod
    def write(destination: str, content: pd.DataFrame) -> None:
        logger.debug(f"Save data to {destination}.")
        content.to_csv(destination, index=False)
...
class JsonFile(BaseFile):
    ...
    def write(destination: str, content: dict) -> None:
        ...
        with open(destination, 'w') as file:
            json.dump(
                content, file, sort_keys=True, indent=4, default=convert
            )
...
def convert_gz2parquet(
    ...
    list_of_files = [
        f for f in Path(str(directory)).glob("*gz")
    ]
    ...
    logger.info(f"Converting {len(list_of_files)} \
        files into {npartitions} {output_format} files...")
